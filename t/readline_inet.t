...
push @tests, [
    ...
    sub {
    	my $c = shift;
	my $blocking = $c->blocking(0);
	defined $blocking or die "Cannot set to nonblocking";
	...
	$b eq "1234567890" || die "\\size failed ($b)";
	$/ = '';
	while(1) {
		$b = <$c>;
		last if defined $b;
...
my $testid = "Test00";
foreach my $test (@tests) {
    my $to_server = IO::Socket::INET->new( 
	PeerAddr => $addr,
	) || do {
